...
class Brands extends ADMIN_Controller {
    ...
    public function restore($id){
        ...
        $this->session->set_flashdata('msg', 'brand restored successfully!');
            ...
            foreach($langs as $key=>$lang){
                ...
                if($image['upload'] == true || $_FILES[$input]['size']<1){
                    ...
                    if((isset($_FILES[$input]) && $_FILES[$input]['size'] > 0)){
                        ...
                        $this->image_thumb($image['full_path'],'./resources/uploads/brands/actual_size/',10,10);
                    }
    ...
    public function check_brand($title){
        ...
        if(!empty($title)) {
            if ($result->num_rows() > 0) {
                $this->form_validation->set_message('check_brand', 'This brand already exist.');
                return false;
            } else {
                return true;
            }
    ...
    public function status($id,$status){
        ...
        $this->db->where('id',$id);
        $this->db->update('brands',$dbData);
        $this->session->set_flashdata('msg','brand status updated successfully!');
        redirect('admin/brands');
    }
    /**
     * loads the add view, then handles the submitted data
        ...
        if(!empty($title)) {
            if ($result->num_rows() > 0) {
                ...
                if($result->id == $id){
                    ...
                    $this->form_validation->set_message('check_brand_edit', 'This brand already exist.');
                    return false;
                }
            } else {
                return true;
