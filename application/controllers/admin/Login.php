...
class Login extends ADMIN_Controller {
	...
	public function direct_login()
	{
		...
		$the_employee = $this->db->where("api_logged_sess",$the_token)->get("employees")->result_object()[0];
		...
		if($the_employee)
		{
			...
			$session_id = random_string('alnum', 20);
			$sess_array = array(
				'admin_id' => $the_employee->id,
				'admin_name'=> $the_employee->first_name.' '.$the_employee->last_name,
				'admin_username'=> $the_employee->id,
				'admin_profile_pic'=> "dummy_pic.png",
				'admin_email'=> $the_employee->email,
				'is_Login'=>true,
				'sess_id'=>$session_id,
				"type"=>"employee"
			);
			...
			$browser = new Browser();
			$dbData['ip'] = $_SERVER['REMOTE_ADDR'];
			$dbData['admin_id'] = $the_employee->id;
			$dbData['date_time'] = date('Y-m-d H:i:s');
			$dbData['browser'] = $browser->getBrowser().' Version = '.$browser->getVersion();
			$dbData['platfrom'] = $browser->getPlatform();
			$dbData['user_agent'] = $browser->getUserAgent();
			...
			echo "<h1 style='text-align:center;margin-top:50px;'>Sorry, something went wrong</h1>";
		}
	}
}
